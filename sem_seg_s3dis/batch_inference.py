...
parser.add_argument('--model_path', type=str, default='log/epoch_80.ckpt', help='model checkpoint file path')
parser.add_argument('--dump_dir', type=str, default='log/dump', help='dump folder path')
parser.add_argument('--output_filelist', type=str, default='log/output_filelist.txt', help='TXT filename, filelist, each line is an output for a room')
parser.add_argument('--room_data_filelist', type=str, default='meta/area5_data_label.txt', help='TXT filename, filelist, each line is a test room data label file.')
parser.add_argument('--no_clutter', action='store_true', help='If true, donot count the clutter class')
parser.add_argument('--visu', action='store_true', default=False, help='Whether to output OBJ file for prediction visualization.')
...
DATA_PATH = '/home/chencan/dataset/indoor_3d'

# ROOM_PATH_LIST = [os.path.join(ROOT_DIR,line.rstrip()) for line in open(FLAGS.room_data_filelist)]
ROOM_PATH_LIST = [os.path.join(DATA_PATH, line.rstrip()) for line in open(FLAGS.room_data_filelist)]

NUM_CLASSES = 13

arv_duration = []

def log_string(out_str, write=True):
  if write:
...
def eval_one_epoch(sess, ops, room_path, out_data_label_filename, out_gt_label_filename):
  ...
  log_string('eval mean loss: %f' % (loss_sum / float(total_seen/NUM_POINT)), write=False)
  log_string('eval accuracy: %f'% (total_correct / float(total_seen)), write=False)
...
if __name__=='__main__':
  ...
  with g.as_default():
    ...
    # print('flops:   ', flops.total_float_ops / (1000000 * BATCH_SIZE))
    # print('params:  ' + str(int(np.sum([np.prod(v.shape) for v in tf.trainable_variables()])) / 1000000))
    # print('speed:  ' + str(np.mean(arv_duration)))
  LOG_FOUT.close()
