...
def get_sampled_edge_feature_separate(point_cloud, old_point_cloud, nn_idx, k=20):
    """Construct edge feature for each point
      Args:
        point_cloud: (batch_size, num_points, 1, num_dims)
        nn_idx: (batch_size, num_points, k)
    ...
    concat = tf.concat([point_cloud_central, point_cloud_neighbors - point_cloud_central], axis=-1)
    edge_feature = point_cloud_neighbors - point_cloud_central
    return edge_feature, point_cloud_central, concat




def get_triangle_feature(point_cloud, nn_idx, k=20):
  """Construct edge feature for each point
  Args:
    point_cloud: (batch_size, num_points, 1, num_dims)
    nn_idx: (batch_size, num_points, k)
  ...
  nearest_neighbor = point_cloud_neighbors[:, :, 0, :]
  nearest_neighbor = tf.expand_dims(nearest_neighbor, axis=-2)
  nearest_neighbor = tf.tile(nearest_neighbor, [1, 1, k, 1])

  triangle_feature = tf.concat([point_cloud_central, nearest_neighbor, point_cloud_neighbors-point_cloud_central, point_cloud_neighbors-nearest_neighbor], axis=-1)
  return triangle_feature